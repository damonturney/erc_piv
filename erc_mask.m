function [maske,x,y,u,v]=erc_mask(ima,wocofile,x,y,u,v)
%
...
while inp~=0,
    ...
    disp('Double click on the polygon to finish, or right click on the polygon and choose Create Mask.')
    disp('Press <BACKSPACE> to remove the previously selected vertex.')
    disp('If you want an empty mask then create one vertex then <BACKSPACE> it.')
...
D=dir(wocofile);
if size(D,1)==1
    if ~isempty(maske.msk)
        for i=1:length(maske)
        mapp=load(D(1).name);
        [maske(i).idxw,maske(i).idyw]=erc_pixel2world(double(maske(i).idx),double(maske(i).idy),...
          double(maske(i).idx),double(maske(i).idy),mapp.comap);
        end
    else
        disp('mask is empty')
    end
